...
export default {
          ...
          转速: {
            ...
            axisLine: {
              lineStyle: {
                ...
                width: (this.item.axisLineWidth * 2) / 3,
                shadowColor: "#fff",
                shadowBlur: 10,
              },
            },
  ...
  watch: {
    ...
    "item.axisLineWidth": function () {
      ...
      this.chartSettings.seriesMap["转速"].axisLine.lineStyle.width = (this.item.axisLineWidth * 2) / 3;
      this.chartSettings.seriesMap["油表"].axisLine.lineStyle.width = (this.item.axisLineWidth * 2) / 3;
    },
  ...
  created: function () {
    ...
    setInterval(function () {
      ...
      if (self.chartData.rows[1].value >= 0 && self.chartData.rows[1].value <= 8) {
        self.chartData.rows[1].value += Math.ceil((Math.random() - 0.5) * 10);
      } else {
        self.chartData.rows[1].value = 4;
      }
      if (self.chartData.rows[2].value >= 0 && self.chartData.rows[2].value <= 2) {
        self.chartData.rows[2].value += Math.ceil((Math.random() - 0.5) * 20) / 10;
      } else {
        self.chartData.rows[2].value = 1;
      }
    }, 2000);
  },
  mounted: function () {
