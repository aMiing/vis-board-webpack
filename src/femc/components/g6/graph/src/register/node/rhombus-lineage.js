...
const _option = {
  ...
  getPath(cfg) {
    const size = cfg.size || [112, 56]; // 如果没有 size 时的默认大小
    const width = size[0];
    const height = size[1];
    //  / 1 \           1        2
    // 4     2
    //  \ 3 /        4         3
    const path = [
      ['M', 0 - (width * 13) / 32, 0 - height / 2], // 上部顶点
      ['L', (width * 19) / 32, 0 - height / 2], // 右侧顶点
      ['L', (width * 13) / 32, height / 2], // 下部顶点
      ['L', 0 - (width * 19) / 32, height / 2], // 左侧顶点
      ['Z'], // 封闭
    ];
    return path;
  },
