...
export default {
  props: ['vfg', 'model', 'schema', 'formOptions', 'disabled', 'readonly', 'rules'],
  ...
  methods: {
    convertValidator(validator) {
      if (isString(validator)) {
        if (this.rules[validator] != null) return this.rules[validator];
        else {
    ...
    validate(calledParent) {
      ...
      // TODO 单项需要使用promise validate时，不能单项设置，只能整个表单设置
      if (!validateAsync) {
    ...
    getItemProp(item, prop) {
      return this.selectOptions[prop] ? item[this.selectOptions[prop]] : item;
    },
  },
};
