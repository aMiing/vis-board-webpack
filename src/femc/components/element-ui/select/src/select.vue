  ...
  methods: {
    ...
    getOption(value) {
      ...
      if (this.$parent && this.$parent.linearData) {
        const { linearData, mergedOptions } = this.$parent;
        const selectedItem = linearData.find(item => item[mergedOptions.value] === value);
        if (selectedItem && selectedItem[mergedOptions.label])
          label = selectedItem[mergedOptions.label];
      }
      let newOption = {
        value: value,
        currentLabel: label,
