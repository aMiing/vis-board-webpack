    ...
    <transition name="el-zoom-in-top" @after-leave="handleDropdownLeave">
      ...
      >
        <g-cascader-panel
          ...
          :virtual-scroll-limit="virtualScrollLimit"
          :render-label="$scopedSlots.default"
          @expand-change="handleExpandChange"
          @close="toggleDropDownVisible(false)"
        >
          <template v-if="$slots['panel-empty']" #empty>
            <slot name="panel-empty" />
          </template>
        </g-cascader-panel>
        <el-scrollbar
          v-if="filterable"
...
import GCascaderPanel from './cascader-panel';
  ...
  methods: {
    ...
    computePresentText() {
      ...
      if (!this.isEmptyValue(checkedValue)) {
        ...
        const pick = node && (config.checkStrictly || node.isLeaf);
        this.presentText = pick ? node.getText(this.showAllLevels, this.separator) : checkedValue;
        return;
      }
    ...
    deleteTag(tag) {
      // 兼容回显找不到的情况
      if (!tag.node) {
        this.checkedValue = this.checkedValue.filter(item => item !== tag.key);
        this.$emit('remove-tag', tag.key);
        return;
      }
    ...
    updateStyle() {
      ...
      if (tags) {
        ...
        const height = Math.max(offsetHeight + 6, inputInitialHeight);

        // 解决缩放抖动的问题
        if (Math.abs(Number(inputInner.style.height) || 0 - Number(height)) > 3) {
