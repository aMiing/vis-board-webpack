...
export default {
  ...
  computed: {
    ...
    // 判断是否需要开启虚拟滚动
    needVirtualScroll() {
      return this.nodes.length > this.panel.virtualScrollLimit;
    },
  },
  ...
  methods: {
    ...
    renderVirtualNodeList(h) {
      ...
      return [
        <recycle-scroller
          items={this.nodes}
          item-size={this.panel.itemHeight}
          key-field="uid"
          scopedSlots={{
            default: ({ item, index }) => {
              const { hasChildren, uid } = item;
              return (
                <cascader-node
                  key={uid}
                  node={item}
  ...
  render(h) {
    ...
    return (
      <el-scrollbar
        ...
        view-class={{
          '--virtual-scroll': this.needVirtualScroll,
      ...
      >
        {isEmpty
          ...
          : this.needVirtualScroll
          ? this.renderVirtualNodeList(h)
          : this.renderNodeList(h)}
      </el-scrollbar>
    );
  },
};
...
<style lang="scss">
.--virtual-scroll.el-cascader-menu__list {
  height: 100%;
