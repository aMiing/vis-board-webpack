...
export default {
  ...
  methods: {
    ...
    expandNodes(nodes) {
      nodes.forEach(node => node && this.handleExpand(node, true /* silent */));
    },
          ...
          if (
            ...
            if (!checkedNode.data[leafKey]) {
              this.lazyLoad(checkedNode, () => {
                checkedNode && this.handleExpand(checkedNode);
              });
            }

            if (this.loadCount === this.checkedValue.length) {
              this.$parent.computePresentText && this.$parent.computePresentText();
            }
          }
        }
